      subroutine glclwr(mdltype,geomnm,dsetnm,parm,lvlt,nolev,rlvl
     *, lvtitle,dtgdb,rtau,units,clastyp,glob,len,istat)
C
C.............................START PROLOGUE............................
C
...
C
C  MODULE NAME:  glclwr
C
C  DESCRIPTION:  Clone glwr. 
C
...
C
C  USAGE:  call glclwr(mdltype,geomnm,dsetnm,parm,lvlt,nolev,rlvl
C                ,lvtitle,dtgdb,rtau,units,clastyp,glob,len,istat)
C       
C
C  PARAMETERS:
...
c
        llen= (len+1)/2
        llen= 8*llen
        open(unit=iunfld,file=fullpath,form='unformatted'
...
c
        if(istat.eq.0) then
          ...
          if(istat.ne.0) then
            print *,' error writing IEEE field'
            istat=1
          endif
        else
          print *,' error opening output unit 80'
          istat=2
        endif
c
        close(iunfld)
