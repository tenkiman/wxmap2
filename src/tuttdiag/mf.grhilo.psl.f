...
c         
      ...
      dimension tlats(nc),tlons(nc),tvals(nc),tdists(nc)
...
c--       get first guess long from vrt925 fix
c
      slat0=slat
      slon0=slon
...
ccc          sdistn1=distsp(tlat,tlon,flat,flon)
          ...
          if(sdistn.lt.sdistmin) then
            ...
            if(verb) print*,'FFF000 ',nstm,tlats(nstm),tlons(nstm),tvals(nstm),tdists(nstm)
...
c--       find the closest low
c
        if(nstm.ge.1) then

          tdistsmin=1e20

          if(verb) print*,'FFF111 N storms in the box: ',nstm,' ib,ie,jb,je: ',ib,ie,jb,je,' tau: ',ktau

          do n1=1,nstm
            if(tdists(n1).lt.tdistsmin) then
              ndist=n1
              tdistsmin=tdists(n1)
              if(verb) print*,'FFF222',n1,tdists(n1),tdistsmin,ndist
            endif
          enddo
          ...
          if(verb) print*,'FFF333',slat,slon,spsl,sdist,sdistpsl
...
c         
          ...
          do j=jb,je
            do i=ib,ie
              call rumdirdist(slat,slon,ygrd(j),xgrd(i),rhead,sdist)
     ...
     $           'FFF444 ndist: ',ndist,' slat/slon/spsl/sdist:',slat,slon,spsl,sdist,
