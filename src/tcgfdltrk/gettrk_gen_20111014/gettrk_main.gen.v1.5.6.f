...
c             flag will have a value of 'U', for "undetermined".
     ...
     &                   ,xval(3),trkrinfo,isastorm(3),isiret3)
                    ...
                    if (trkrinfo%type == 'tcgen') then
                      ! This next bit of code adds a second layer of 850 mb
                      ! Vt magnitude checking.  This is to decrease the 
                      ! occurrence of interrupted tcgen tracks, which
                      ! occasionally happens for weak storms when this
                      ! criterion is not met for one time period.  So in 
                      ! this next code, we check to see if the vt850_flag
                      ! was 'y' for at least 75% of the time over the 
                      ! last 24h.  For time periods shorter than 24h 
                      ! (e.g., the storm was just detected at 144h and 
                      ! we are now at 156h), the threshold is still that
                      ! for at least 75% of the time the system has been
                      ! detected as a storm, it must have a vt850_flag 
                      ! value of 'y'.
     ...
     &                                    cc_time_sum_tot
                          ...
                          if (cc_time_pct >= 0.75) then
                            ...
                            print *,'    this hour was NO, but a check'
                            print *,'    of recent vt850_flags '
                            ...
                            print *,'!!! NOTE ON Vt_850 CHECK: The '
                            print *,'!!! isastorm flag returned for '
                            print *,'    this hour was NO, and a check'
                            print *,'    of recent vt850_flags '
     ...
     &            calcparm(1,ist) .and. stormswitch(ist) == 1) then
                  ...
                  if (atcfname == 'GFSO' .and.
     &                abs(slatfg(ist,ifh)) >= 25.0) then
                    max_mslp_850 = 405.0
                  else if (atcfname == 'GFSO' .and.
     &                abs(slatfg(ist,ifh)) < 25.0) then
                    max_mslp_850 = 405.0
                  else
                    max_mslp_850 = 323.0
...
c           and hurricane force winds.
     ...
     &                       ,xmaxwind(ist,ifh),trkrinfo,rmax,igmwret)
              if (igmwret /= 0 .and. gridmove_status == 'stopped') then
                print *,' '
                print *,'!!! Return code from get_max_wind is /= 0. '
                print *,'!!! rcc= igmwret= ',igmwret
                print *,'!!! Also, this is a moveable, regional grid'
                print *,'!!! and the grid did not change from the last' 
                print *,'!!! lead time to the current one, so what has'
...
c             print *,'!!! jbeg= ',jbeg,' jend= ',jend,' jmax= ',jmax
              ...
              if (ibeg < 1) then
                if (trkrinfo%gridtype == 'global') then
                  ...
                  print *,'!!!         '
                  print *,'!!! Vmag will not be computed for this time.'
                  print *,' '
                  ifmret = 99
                  return
                endif
