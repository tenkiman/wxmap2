...
c
     ...
     &                        ,iret
      ...
      call open_grib_files (lugb,lugi,lout,iret)
...
c
      ...
      integer   pdf_ct_tot,pdf_ct_bin(16)
...
c
  ...
  402     format (1x,'*   New forecast hour: ',i4,':',i2.2)
        ...
        call getgridinfo (imax,jmax,ifh,dx,dy,trkrinfo
...
c       then call subroutine to read data for this forecast time.
     ...
     &               ,need_to_flip_lats,need_to_flip_lons,inp)
...
c           to get a guess position for the next forecast hour.
  ...
  432                 format (1x,'!!! Fcst hr  = ',i4,':',i2.2)
     ...
     &                    ,-999.0,inp,ist
     &                    ,ifcsthour,0.0
     &                    ,0.0,vradius,maxstorm,trkrinfo,-99.0,ioaxret)
                      imeanzeta = -99
                      igridzeta = -99
...
c           knots (1.9427) is explained in output_atcf.
  ...
  442         format (1x,'forecast hour= ',i4,':',i2.2,' follow:')
     ...
     &                      ,int((xmaxwind(ist,ifh)*1.9427) + 0.5)
              ...
              ! Only call output routines every 6 hours....

              ileadtime = nint(fhreal(ifh) * 100.0)
              leadtime_check = mod(ileadtime,600)
...
c
  73  format ('fixpos  ',a4,'  fhr= ',i4,':',i2.2,'   Fix position=  '
     &       ,f7.2,'E  (',f6.2,'W)',2x,f7.2,'   Max Wind= ',i3,' kts')
      ...
      deallocate (hgt); deallocate (slp); deallocate (tmean)
      deallocate (masked_out); deallocate (masked_outc)
      if (allocated(cpshgt)) deallocate (cpshgt)
c
      return 
      end   
...
c
c-----------------------------------------------------------------------
      subroutine open_grib_files (lugb,lugi,lout,iretverb)
...
c     will just have it use the Hart cyclone phase space (CPS) scheme.
      ...
      integer  imax,jmax,igpret,igcpret,ist,ifh,maxstorm,igvpret
...
c
 ...
 615    format(1x,'#---------------------------------------------------#')
  ...
  73  format ('cps_stats: ',a4,'  lead time= ',i3,':',i2,'   paramb= '
     ...
     &                  ,fixlon,fixlat,valid_pt,maxstorm,igvpret)
      endif
...
c     ------------------------------------------------------------------
      ...
      if ( verb .ge. 3 ) then
        print *,' '
        print *,'++ In get_cps_vth, values for vth follow for lead time= '
...
c              barnes analysis.
      ...
      character*1 ccflag,get_last_contour_flag
      real      fixlon(maxstorm,maxtime),fixlat(maxstorm,maxtime)
...
c     ------------------------------------------------------------
      ...
      wcore_trkrinfo%contint = 1.0  ! ...except specify contint as 1
...
c     the call to check_closed_contour below.
     ...
     &     ,trkrinfo,ifilret)
        if (ifilret == 0) then
          if ( verb .ge. 3 ) then
            ...
            write (6,105) storm(ist)%tcv_storm_id,gstorm(ist)%gv_gen_date
     ...
     &         ,wcore_mean_lat,wcore_mean_val
            write (6,106) storm(ist)%tcv_storm_id,gstorm(ist)%gv_gen_date
     &         ,gstorm(ist)%gv_gen_fhr,gstorm(ist)%gv_gen_lat
     &         ,gstorm(ist)%gv_gen_latns,gstorm(ist)%gv_gen_lon
     &         ,gstorm(ist)%gv_gen_lonew,gstorm(ist)%gv_gen_type
     &         ,ifhours(ifh),ifclockmins(ifh)
     &         ,ifix,jfix,wcore_point_max
          ...
          if ( verb .ge. 1 ) then
            ...
            print *,'!!! i and j went out of bounds for a regional grid.'
            print *,'!!! This should have been caught in a previous call'
            print *,'!!! to find_maxmin for one of the various fix parms.'
            print *,'!!! In any event, we are STOPPING.'
            print *,' '
          endif
          STOP 97
        endif
...
c     surrounding the warm core (using a 1 mb interval), that
...
c     the tlastcont and rlastcont values are returned.
     ...
     &    ,valid_pt,wcore_mask,ccflag,'max',wcore_trkrinfo
...
c       sr_vt:    Quadrant tangential winds in storm-relative framework
      ...
      USE inparms; USE phase; USE set_max_parms; USE tracked_parms
      USE def_vitals; USE trig_vals; USE verbose_output
...
c     -----------------------------------------------------------------
     ...
     &                  ,dx,dy,'u',xintrp_u,ibiret1)

          call bilin_int_uneven (targlat,targlon,rdist(idist)
     &                  ,dx,dy,'v',xintrp_v,ibiret2)
...
c       rdist     Radii (km) at which the winds will be evaluated
      ...
      character*5 :: cbin(5) = (/'0-100','0-200','0-300','0-400'
     &     ,'0-500'/)
...
c     sdp   Storm surge damage potential
      ...
      integer  ibeg,jbeg,iend,jend,igiret,i,j,maxstorm
      real     fixlon(maxstorm,maxtime),fixlat(maxstorm,maxtime)
      real     ike(max_ike_cats)
...
c
      ...
      USE grid_bounds; USE tracked_parms; USE level_parms; USE verbose_output
...
c
      ...
      if (ifh > 1) then
        ...
        do ist = 1,maxstorm
          ...
          if ( verb .ge. 3 ) then
            write (6,81) ist,prstemp(ist)/100.0
          endif
        enddo

        imode = 2
        sortindex = 0
        call orders_4 (imode,iwork,prstemp,sortindex,maxstorm,1,8,1)
        ...
        do ist = 1,maxstorm
          if ( verb .ge. 3 ) then
            if (prstemp(sortindex(ist))/100.0 < 900000.) then
  ...
  82    format (1x,'ist= ',i5,'  sortindex(ist)= ',i5,' prstemp= ',f7.2)
...
c
      ...
      integer intlon,intlat,irmax,output_fhr
      integer vradius(3,4)
...
c
      ...
      USE gen_vitals; USE verbose_output
c
      type (trackstuff) trkrinfo
...
c      numalloc = jnum * inum 
C  mf 20100327 - increase allocation...
...
c       -------------------
        ...
        imode = 2
        do  m=1, quadct(k)
          isortix(quadct(k)) = 0.0
        enddo
        call orders_4 (imode,iwork,dtemp,isortix,quadct(k),1,8,1)
...
c           gotten to this point in threshloop).
        ...
        deallocate(isortix,stat=idta)
        deallocate(iwork,stat=iwa)
        if (iisa /= 0 .or. idta /= 0 .or. iwa /= 0) then
          if ( verb .ge. 1 ) then
            print *,' '
            print *,'!!! ERROR in getradii deallocating isortix or dtemp or iwork'
            print *,'!!! array for quadrant= ',k,' iisa = ',iisa
            print *,'!!! idta= ',idta,' iwa= ',iwa
          endif
          itret = 94
      ...
      if (iqa /= 0) then
        if ( verb .ge. 1 ) then
          print *,' '
          print *,'!!! ERROR in sub getradii deallocating quadinfo array.'
          print *,'!!! iqa = ',iqa
        endif
        igrret = 94
        return
...
c
      ...
      USE atcf; USE gen_vitals; USE tracked_parms; USE verbose_output
c
      type (datecard) inp
c
...
c         
     ...
     &    iend < 1 .or. jend < 1) then
        if ( verb .ge. 1 ) then  
          print *,' '
          print *,'ERROR in get_uv_center calculating ibeg, iend, jbeg or'
      ...
      if (iend > imax) then
        if (trkrinfo%gridtype == 'global') then
          ...
          if ( verb .ge. 3 ) then
            print *,' '
            print *,'!!! NOTE: In get_uv_center, the iend returned from '
            print *,'!!! get_ij_bounds is > imax, and our gridtype is NOT'
            print *,'!!! global, so we will redefine iend to imax.'
            print *,' '
          endif
          iend = imax
        endif
      ...
      if (numinterp == 0) then
        ...
        if (iend > imax) then
          if (trkrinfo%gridtype == 'global') then
            ...
            if ( verb .ge. 1 ) then
              print *,' '
              print *,'ERROR in get_uv_center:  Should not have gotten to'
              print *,'this point in get_uv_center for a regional grid; '
              print *,'iend should not > imax here!!!'
            endif
            igucret = 99
            return
          endif
...
c         parameter retrk_vmag, and use this instead.
      ...
      else if ( (grdspc < 1.26).and.(grdspc >= 0.40) ) then
        rads = rads_most; re = retrk_most; ri = ritrk_most
      else if ( (grdspc < 0.40).and.(grdspc >= 0.10) ) then
 ...
 46       format (' vmag_latmin= ',f8.3,' vmag_lonmax= ',f8.3
     &           ,'E  (',f8.3,'W)')
        ...
        if (vmag_lonmin > 330. .and. vmag_lonmax < 30.) then
          ! For a case of searching for vmag in which the vmag subgrid
          ! straddles the GM, don't even bother with trying to figure
          ! it out.  Just report back that the vmag was unable to 
          ! be reported.
          if ( verb .ge. 3 ) then
            print *,' '
            print *,'!!! WARNING: For a case of find_maxmin, our vmag'
            print *,'!!! subgrid is straddling the GM, so we will abort'
            print *,'!!! the search for vmag at this lead time.'
            print *,' '
          endif
          ifmret = 92
          return
...
c        vlat(jix) = rlatt
        ...
        iloop: do i=-npts,npts,bskip1
          ...
          if (cparm == 'vmag') then
            if ( verb .ge. 3 ) then
              print *,' '
              print '(a16,i6,a4,i6,2(a8,f8.3),a12,f8.3)'
     &              ,'in find_max, i= ',i
     &              ,' j= ',j,' rlatt= ',rlatt,' rlont= ',rlont
     &              ,' 360-rlont= ',360.-rlont
            endif
          endif
...
c         subroutine.
          ...
          if (rlont >= (grid_maxlon + dx - grid_buffer)) then
            if (trkrinfo%gridtype == 'global') then
              if (cparm == 'vmag') then
                if ( verb .ge. 1 ) then
                  print *,'ERROR:  SHOULD NOT BE IN HERE!!! AAA'
                endif
          ...
          if (rlont < (temp_grid_minlon + grid_buffer)) then
            if (trkrinfo%gridtype == 'global') then
              if (cparm == 'vmag') then
                if ( verb .ge. 1 ) then
                  print *,'ERROR:  SHOULD NOT BE IN HERE!!! BBB'
                endif
                cycle iloop  ! We are off the small vmag subgrid
              else
                rlont = rlont + 360.  ! We just GM-wrapped for the full,
     ...
     &        rlatt < (grid_minlat + grid_buffer)) then
            if (cparm == 'vmag') then
              if ( verb .ge. 1 ) then
                print *,'ERROR:  SHOULD NOT BE IN HERE!!! CCC'
              endif
            endif
            cycle iloop
          endif
...
c         and go to the next point.
     ...
     &        ,grid_minlon,trkrinfo,icvpret)
          ...
          if (cparm == 'vmag') then
            if ( verb .ge. 3 ) then
              print '(a41,i6,a4,i6,2(a8,f8.3),a12,f8.3,2(a7,f10.3))'
     &              ,'in find_max, MADE IT PAST DIST CHECK, i= ',i
     &              ,' j= ',j,' rlatt= ',rlatt,' rlont= ',rlont
     &              ,' 360-rlont= ',360.-rlont,' rads= ',rads
     &              ,' dist= ',dist
            endif
          endif
...
c           size array to send to barnes (maxvgrid=8 means 8x8)
            ...
            if ( verb .ge. 3 ) then
              print '(a17,f8.3,a8,f8.3,a11,i5)','  + vmag_latmax= '
     &                    ,vmag_latmax
     &                    ,' rlatt= ',rlatt,' jvlatfix= ',jvlatfix
              print '(a17,f8.3,a18,f8.3,a11,i5)','  + vmag_lonmin= '
     &                    ,vmag_lonmin,' 360-vmag_lonmin= '
     &                    ,360.-vmag_lonmin,' ivlonfix= ',ivlonfix
              print '(a11,f8.3,a12,f8.3,a11,i5)','  + rlont= '
     &                    ,rlont,' 360-rlont= '
     &                    ,360.-rlont,' ivlonfix= ',ivlonfix
            endif
            ...
            if ( verb .ge. 3 ) then
              print '(a10,i4,a11,i4,a7,i4)','  + ibeg= ',ibeg
     &                    ,' ivlonfix= ',ivlonfix
     &                    ,' iend= ',iend
              print '(a10,i4,a11,i4,a7,i4)','  + jbeg= ',jbeg
     &                    ,' jvlatfix= ',jvlatfix
     &                    ,' jend= ',jend
            endif
     ...
     &          iend > imax .or. jend > jmax) then
              ...
              ! bounds.  Rather, just set the violating bound(s) to the 
              ! min or max limit.
          ...
          if (cparm == 'vmag') then
            ri = re * 3
            if ( verb .ge. 3 ) then
              print '(a31,f10.4,a6,f10.4)'
     &             ,'  + before call to barnes, re= ',re,'  ri= ',ri
              print '(a33,f10.4)','  + vmag at ivlonfix, jvlatfix = '
     &                   ,fxy(ivlonfix,jvlatfix)
            endif
          endif
     ...
     &     ,trkrinfo,iret)
          ...
          if (cparm == 'vmag') then
            if ( verb .ge. 3 ) then
              print '(a14,i4,a11,i4,a15,f12.5)','  + ivlonfix= ',ivlonfix
     &                   ,' jvlatfix= ',jvlatfix,' barnes value= ',ftemp
            endif
          endif
c
          if (maxmin == 'max') then
...
c
      USE def_vitals; USE grid_bounds; USE set_max_parms; USE verbose_output 
c
...
c
      type (datecard) inp
c
      integer, parameter :: jf=40000000,lugb=11,lugi=31
      integer, parameter :: nparms=14
      real, allocatable :: f(:)
      real :: dmin,dmax
      ...
      integer   igvret,ifa,ila,ip,ifh,i,j,k,kj,iret,kf
c
      lbrdflag = 'n'
...
c
  ...
  29        format (' rec#  parm# levt lev  byy   bmm  bdd  bhh  fmin  np'
     &             ,'ts  minval       maxval') 
...
c         once since using same model for all variables).
     ...
     &                                           ,need_to_flip_lats)
                if ( verb .ge. 3 ) then
                  print *,'test flip: u(1,50,1) = ',u(1,50,1)
                endif
...
c     *------------------------------------------------------------*
  ...
  39            format (' rec#  parm# levt lev  byy   bmm  bdd  bhh  fmin'
     &             ,'   npts  minval       maxval')
...
c
      ...
      USE structure; USE verbose_output
c
      implicit none
c
      integer ifh
...
c
  ...
  815 continue
      read (5,NML=verbose,END=819,ERR=823)
  819 continue
      goto 827
  823 continue
      verb = 1
      ...
      do i = 1,ifhmax
        ...
        if (i > 1) then
          if (fhreal(i) > fhreal(i-1)) then
            ...
            if ( verb .ge. 1 ) then
              print *,' '
              print *,'!!! ERROR: In read_fhours, the time read in is not'
              print *,'!!! greater than the previous time.'
              print *,'!!! i= ',i
...
c------
      ...
      do while (.true. .and. ii <= maxstorm_tc)
        if ( verb .ge. 3 ) then
          print*,'qqqqqqqqqqqqqqqqqq ',ii,maxstorm_tc
        endif
        read (lucard,21,END=801,ERR=801) tmpstorm(ii)
        ii = ii + 1
      enddo
...
C---      mf 2010031400 -- iret should be set to 0, not assumed to initialized to 0
C        
        iret=0
        return
...
c
c---------------------------------------------------------------------
      subroutine getgridinfo (imax,jmax,ifh,dx,dy,trkrinfo
     &                ,need_to_flip_lats,need_to_flip_lons,inp,iggret)
c
...
c
      ...
      logical(1) :: need_to_flip_lats,need_to_flip_lons
      logical(1), allocatable :: lb(:)
      integer, parameter :: lugb=11,lugi=31,jf=40000000
...
c     check the value of the GRIB scanning mode flag here.
      ...
      if ( verb .ge. 3 ) then
        print *,'FLIP: iscanflag= ',iscanflag
      endif
      if (mod(iscanflag,128) >= 64) then
        ...
        if ( verb .ge. 3 ) then
          print *,'FLIP: need_to_flip_lats = TRUE'
          print *,'    flip: glatmin= ',glatmin,' glatmax= ',glatmax
        endif
      else
        ...
        if ( verb .ge. 3 ) then
          print *,'NO FLIP: !!!!!! need_to_flip_lats = FALSE'
          print *,'    no flip: glatmin= ',glatmin,' glatmax= ',glatmax
        endif
      endif
...
c
      ...
      real       xplon,yplat,dmin,dmax,dx,dy
      real       grid_maxlat,grid_minlat,grid_maxlon,grid_minlon
...
c     LOCAL VARIABLES:
c
      USE tracked_parms; USE trig_vals; USE grid_bounds; USE verbose_output
...
c     ifamret  return code from this subroutine
      ...
      USE trkrparms; USE set_max_parms; USE contours; USE verbose_output
...
c              not if we just delete them now.
      ...
      integer    search_next_i((imax*jmax)/2)
      integer    search_next_j((imax*jmax)/2)
      integer    next_contour_i((imax*jmax)/2)
      integer    next_contour_j((imax*jmax)/2)
      integer    beyond_contour_i((imax*jmax)/10)
      integer    beyond_contour_j((imax*jmax)/10)
      integer    hold_mask_i_loc((imax*jmax)/2)
      integer    hold_mask_j_loc((imax*jmax)/2)
      integer    temp_mask_i_loc((imax*jmax)/2)
      integer    temp_mask_j_loc((imax*jmax)/2)
...
c     instead of the meaningless point 0 (i=0).
      ...
      SUBROUTINE ORDERS_4 (IN,ISORT,IDATA,INDEX,N,M,I1,I2)
  
      INTEGER(4)  IDATA(M,N),ICHEK,IBYT,INDEX(N),ISORT(N),INDX(0:255),KNDX(0:255)
      REAL(4)     SMAL,RCHEK
