 ...
 *
 * IDENTIFICATION:
 *    get_grid__1deg_.c - Gets a NOGAPS file from the TEDS database (1.0 deg)
 *
 *            (1,181)--90N---> (360,181)
 *                      .
 *                      .
 ...
 **********************************************************************/
   ...
   get_grid_1deg(icentury,idate, presl, presu, parm, itau, gfield, sflag) 
...
#if 0   
   printf("in get_grid_1deg,presl=%d,presu=%d,parm=%d,itau=%d \n",*presl,*presu,*parm,*itau);
   printf("in get_grid_1deg,icentury,date=%d %d %d %d %d\n",*icentury,idate[0],idate[1],idate[2],idate[3]);
#endif
...
/**************************************/
   ...
   stGridFormat.lMaxXPoint         = 360;
   stGridFormat.lMaxYPoint         = 181;
