...
c
      ...
      character*8  strmid
      character*10 aymdh
      character*50 stm_path
      character*75 input_file, output_file
c
...
c
      input_file = stm_path(1:lastch(stm_path))//"/"//strmid//".com"
cc      input_file = strmid//".com"     ! uncomment to run locally
c
      open ( 21, file=input_file, status='old', iostat=ios, err=1010 )
c
c**   Read in the compute data and close the file
c
      call getARecord ( 21, "CARQ", comRcd, istat )
      if ( istat .eq. 0 ) goto 1020
c
      close ( 21 )
c
c**   Read in the current data
c
      call getSingleTAU ( comRcd, 0, tauData, istat )
      if ( istat .ne. 1 ) goto 1030
c
...
c
      ...
      dircur = tauData%aRecord(1)%dir
      spdcur = tauData%aRecord(1)%speed
c
c**   Read in the minus 12-hour old data
c
      call getSingleTAU ( comRcd, -12, tauData, istat )
      if ( istat .ne. 1 ) goto 1030
c
      latm12 = tauData%aRecord(1)%lat
      lonm12 = tauData%aRecord(1)%lon
      wndm12 = tauData%aRecord(1)%vmax
      dirm12 = tauData%aRecord(1)%dir
      spdm12 = tauData%aRecord(1)%speed
c
c**   Read in the minus 24-hour old data
c
      call getSingleTAU ( comRcd, -24, tauData, istat )
      if ( istat .ne. 1 ) goto 1030
c
      latm24 = tauData%aRecord(1)%lat
      lonm24 = tauData%aRecord(1)%lon
c
      write ( 6, 1 ) strmid
    1 format (' CLIPer, SHiFoR, AND eXTRaPolation forecast for ',a8,//)
c
c**   compute forecasts
c
...
c
     ...
     &                  dirm12, spdm12, wndcur, strmid )
          call atshif ( iymdh,  latcur, loncur, latm12, lonm12,
     &                  wndcur, wndm12, strmid )
          call atshif5d ( iymdh,  latcur, loncur, latm12, lonm12,
     &                  wndcur, wndm12, strmid )
c
      else
c
     ...
     &                  latm24, lonm24, dircur, spdcur, wndcur, strmid )
          call epshif ( iymdh,  latcur, loncur, latm12, lonm12,
     &                  wndcur, wndm12, strmid )
          call epshif5d ( iymdh,  latcur, loncur, latm12, lonm12,
     &                  wndcur, wndm12, strmid )
c
      endif
c
      call xtrap ( iymdh, latcur, loncur, latm12, lonm12, stmrid )
C
c**   Create the file name and open the file
c
      output_file = stm_path(1:lastch(stm_path))//"/a"//strmid//".tmp"
cc      output_file = "a"//strmid//".tmp"    !  uncomment to run locally
c
      open ( 31, file=output_file, status='unknown' )
c
c**   Write the forecasts to a temporary a-deck for merging with the
c**     actual adeck and close the file
c
      techname = 'CLIP'
      call newWriteAidRcd ( 31, strmid, aymdh, techname, ifpclip )
c
      techname = 'XTRP'
      call newWriteAidRcd ( 31, strmid, aymdh, techname, ifpxtrp )
c
      techname = 'SHFR'
      call newWriteAidRcd ( 31, strmid, aymdh, techname, ishifor )
c
      techname = 'SHF5'
      call newWriteAidRcd ( 31, strmid, aymdh, techname, ishifor5 )
c
      close (31)
c
...
c**   Construct the cdeck file name and open the file
c
      epcof_file = include_path(1:lastch(include_path))//"/epcliper.cof"
cc      epcof_file = "epcliper.cof"    ! uncomment to run locally
c
