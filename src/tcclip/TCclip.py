...
def JTWCcliper(idtg,lat0,lon0,dir,spd,vmax,sid,basin):
    ...
    (latm12,lonm12)=TC.rumltlg(dir,spd,-12.0,lat0,lon0)
    (latm24,lonm24)=TC.rumltlg(dir,spd,-24.0,lat0,lon0)
    print 'MMM-12 lat/lon: ',latm12,lonm12
    print 'MMM-24 lat/lon: ',latm24,lonm24
...
def MakeCliperForecast(dtg,source):

    icards=TC.findtc(dtg,source)

    ntcs=len(icards)

    if(ntcs == 0):
        print "EEE no TCs for: %s  using source: %s"%(dtg,source)
        return(0,0)
    ...
    ##  02S 075S 0827E  2700 030
        ...
        print "FFFFFFFFFFFFFFF",flat[0],flon[0]
