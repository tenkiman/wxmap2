...
C
      ...
      subroutine doWriteAidRcd5 (datFile, stormID, cdtg, techname5, 
     1     itau, llwnd )
...
c     Assign the objective aid technique number
      if( techname5(1:4) .eq. 'CARQ' ) then
         technum = '01'
      else if( techname5(1:4) .eq. 'WRNG' ) then
         technum = '02'
      else
         technum = '03'
      endif
...
cx    only if the lat and lon are meaningful
         if ( ilon .le. 1800 .and. ilat .lt. 900 ) then
            write(datFile,9080) basin, stormnum, cdtg, 
     1           technum, techname5, itau, ilat, ns, ilon, ew, 
     1           llwnd(3)
 9080       format( A2,', ',A2,', ',A10,', ',A2,', ',A5,', ',
     1           I3,', ',I3,A1,', ',I4,A1,', ',I3 )
         endif
      endif
...
C
      ...
      subroutine newWriteAidRcd5 (datFile, stormID, cdtg, techname5, 
     1     ltlnwnd )
...
c     For CARQ and WRNG loop on the taus: -24, -18, -12, -6 and 0
      if (techname5(1:4).eq.'CARQ' .or. techname5(1:4).eq.'WRNG') 
     $     loopend = 5
...
c--       cycle if both lat/lon 0 or vmax 0

         if(llwnd(3).eq.0 .or. (llwnd(1).eq.0 .and. llwnd(2).eq.0) ) cycle

         call doWriteAidRcd5(datFile, stormID, cdtg, techname5, itau, 
     &                      llwnd )
      enddo
C
      END
...
c--       mf cycle if lat/lon = 0 == non forecast
c
          if((ilat.eq.0 .and. ilon.eq.0) .or. (iwnd.eq.0) ) cycle
