...
c              07 Apr 2011 - New predictors added (V000,V850,V500,V300,TG
c                                                                        
C.DUP lsdiag,primd                                                       
      parameter(ixmx=201,iymx=101,ipmx=21)                               
      parameter(irmx=350,iamx=24)                                        
c                                                                        
C.DUP lsdiag,prima                                                       
...
c                                                                        
      ...
      dimension itgrd(0:itmax),itadv(0:itmax)                            
c                                                                        
      ...
      dimension iv000(0:itmax),iv850(0:itmax)                            
      dimension iv500(0:itmax),iv300(0:itmax)                            
c                                                                        
...
c        Extract storm name and date information from header line        
  ...
  102    format(1x,a4,1x,3(i2),1x,i2,1x,i4)                              
         ...
         csnum  = hline(43:44)	                                          
c                                                                        
c        Specify sign of longitude depending on the basin                
         if (cbasin .eq. 'WP') then                                      
            sgnlon =  1.0                                                
         else                                                            
            sgnlon = -1.0                                                
         endif                                                           
c                                                                        
         write(lulog,748) sname,imyear,immon,imday,imtime,cbasin,csnum   
  748    format(/,' HEADER: ',a4,1x,3(i2.2),1x,i2.2,1x,a2,a2)            
c                                                                        
...
C                                                                        
         do 15 k=-2,itmax                                                
            ...
            slon(k) = sgnlon*float(islon(k))/10.0                        
c                                                                        
...
c                                                                        
c           Specific radius (km) for azimuthal average of tangential wind
            radaav = 500.0                                               
            indxaav = ifix(0.0001 + radaav/drcf)                         
...
c                                                                        
            ...
            itlon(kt) = ifix(sgnlon*10.0*sloncf)                         
c                                                                        
...
c                                                                        
                  if (ll .eq. i000) then                                 
c                    Save azimuthally averaged 1000 hPa tangential wind a
                     iv000(kt) = ifix(vtaa(indxaav)*10.0)                
                  endif                                                  
c                                                                        
                  if (ll .eq. i850) then                                 
c                    Save azimuthally averaged 850 hPa tangential wind at
                     iv850(kt) = ifix(vtaa(indxaav)*10.0)                
                  endif                                                  
c                                                                        
                  if (ll .eq. i500) then                                 
c                    Save azimuthally averaged 500 hPa tangential wind at
                     iv500(kt) = ifix(vtaa(indxaav)*10.0)                
                  endif                                                  
c                                                                        
                  if (ll .eq. i300) then                                 
c                    Save azimuthally averaged 300 hPa tangential wind at
                     iv300(kt) = ifix(vtaa(indxaav)*10.0)                
                  endif                                                  
c                                                                        
c                 do kk=0,nrcf                                           
...
c              Vortex lost, so skip the filtering, but still calculate th
c              iv000, iv850, iv500 and iv300 variables.                  
c                                                                        
               irmftem = 0                                               
               call lltoxy(olons,olats,colat,slon(kt),slat(kt),          
     +                     xcftem,ycftem)                                
c                                                                        
               do ll=1,nvp                                               
                  if (ll .eq. i000) then                                 
c                    Save azimuthally averaged 1000 hPa tangential wind a
                     call vremove(uxyp(1,1,ll),vxyp(1,1,ll),rcf,thetacf, 
     +                            xcftem,ycftem,mrcf,mtcf,nrcf,ntcf,     
     +                            uraa,vtaa,irmftem)                     
                     iv000(kt) = ifix(vtaa(indxaav)*10.0)                
                  endif                                                  
c                                                                        
...
c                                                                        
c           Calculate and save the 850 to 700 hPa temperature gradient   
c           and advection                                                
            call tgrad(slat(kt),pvp(i700),uvp(i700),vvp(i700),           
     +                          pvp(i850),uvp(i850),vvp(i850),           
     +                                             tgrd,tadv,ierr)       
c                                                                        
            if (ierr .eq. 0) then                                        
               itgrd(kt) = ifix(tgrd*1.0e+7)                             
               itadv(kt) = ifix(tadv*1.0e+6)                             
            else                                                         
               itgrd(kt) = 9999                                          
               itadv(kt) = 9999                                          
            endif                                                        
...
c                                                                        
c        write(6,817) glat,glon,grsst,gvmax,gshdc,gdelv,gt200,gd200,     
c    +                gspd,ibasin                                        
c 817    format('ginput ',9(f7.1),1x,i2)                                 
c                                                                        
...
c                                                                        
            tlabel='V000'                                                
            write(luout,236) (iv000(k),k=0,itmax),tlabel                 
            tlabel='V850'                                                
            write(luout,236) (iv850(k),k=0,itmax),tlabel                 
            tlabel='V500'                                                
            write(luout,236) (iv500(k),k=0,itmax),tlabel                 
            tlabel='V300'                                                
            write(luout,236) (iv300(k),k=0,itmax),tlabel                 
            tlabel='TGRD'                                                
            write(luout,236) (itgrd(k),k=0,itmax),tlabel                 
            tlabel='TADV'                                                
            write(luout,236) (itadv(k),k=0,itmax),tlabel                 
c                                                                        
...
c       
c     Output:
c       vvavg:  Vertically averaged vertical velocity
