...
c
      parameter (nvar=24,nvarp=3)
      parameter (mft=20)
      dimension ilat(-2:mft),ilon(-2:mft),ishr(0:mft)
      dimension ishtd(0:mft),ivmaxb(0:mft)
      dimension isst(0:mft),idist(0:mft)
      dimension irsst(0:mft),iu200(0:mft)
      dimension it150(0:mft),it200(0:mft),it250(0:mft)
      ...
      dimension irhcn(0:mft),iphcn(0:mft)
      dimension igoes(0:mft),igoesm3(0:mft),igoesxx(0:mft)
c
      dimension iasst(0:mft)
      dimension sst(0:mft),vsst(0:mft),rlat(-2:mft),rlon(-2:mft)
...
c     ++ Common blocks for main iships.f program for lgecal routine
      ...
      common /lgetdi/ ishr,iepos,iz850,id200,ishdc,irhcn,itadv
...
c     Specify intensity (kt) for which storm is considered dissipated
      ...
      ivdiss = ifix(vdiss)
c
c     Set iperts= 1 to add perturbation SHIPS forecasts (satellite input)
c      or iperts= 0 to skip perturbation SHIPS
      iperts=0
c
...
c
      if (ibasin .eq. 2) then
         if (ioper .eq. 1) then
            fnco = trim( coef_location ) // 'ships11_coef_epac.dat'
         else
            fnco = 'ships11_coef_epac.dat'
         endif
         jdmax =238
	 raefld = 30.0
         ishrx =mft
...
c
         ...
         if (ioper .eq. 1) then
            fnco = trim( coef_location ) // 'ships11_coef_atlc.dat'
         else
            fnco = 'ships11_coef_atlc.dat'
         endif
         jdmax =253
	 raefld = 30.0
         ishrx =mft
...
c
 1100 continue
  ...
  111    format(a130)
c
         if (iline(117:120) .eq. 'LAST' ) go to 1200
c
...
c
         iamsr=0
         if (iamsr .eq. 1) then
c           Read AMSR SST line
            if (iline(117:120) .eq. 'AMS1') then
...
c
         if (iline(117:120) .eq. 'TADV') then
            read(iline,116) (itadv(k),k= 0,mft)
         endif
c
         if (iline(117:120) .eq. 'TGRD') then
            read(iline,116) (itgrd(k),k= 0,mft)
         endif
c
         if (iline(117:120) .eq. 'V500') then
            read(iline,116) (iv500(k),k= 0,mft)
         endif
c
         if (iline(117:120) .eq. 'V300') then
            read(iline,116) (iv300(k),k= 0,mft)
         endif
c
         if (iline(117:120) .eq. 'V20C') then
...
c
c     Convert t200 and t250 to real, and apply 250 thershold
      do k=0,mft
...
c        call newWriteAidRcd(luat,strmid,aymdh,"SSGS",ifship)
      endif
c
...
c        call newWriteAidRcd(luat,strmid,aymdh,"SDGS",ifship)
      endif
c
...
c     call newWriteAidRcd(luat,strmid,aymdh,"SLGS",ifship)
c
...
c     **** End LGE Model ****
c
c     **** Begin ET classification section ****
c 
      call etclass(mft,isst,it150,iepos,itgrd,iv500,iv300,ishdc,
...
c     **** End ET classification section ****
c
...
c
c     if (ipgoes .eq. 0) then
c        write(lush,503)
c 503    format(20x,'*      GOES DATA AVAILABLE                  *' )
c     else
c        write(lush,504)
c 504    format(20x,'*      GOES DATA MISSING, PROXY USED        *' )
c     endif
c
c     if (ipohc .le. 1) then
c        write(lush,506)
c 506    format(20x,'*      OHC  DATA AVAILABLE                  *' )
c     else
c        write(lush,507)
c 507    format(20x,'*      OHC  DATA MISSING, PROXY USED        *' )
c     endif
c
      if     (ipgoes .eq. 0 .and. ipohc .le. 1) then
         write(lush,503)
  503    format(20x,'* GOES AVAILABLE,       OHC AVAILABLE       *' )
      elseif (ipgoes .ne. 0 .and. ipohc .le. 1) then
         write(lush,504)
  504    format(20x,'* GOES PROXY USED,      OHC AVAILABLE       *' )
      elseif (ipgoes .eq. 0 .and. ipohc .gt. 1) then
         write(lush,506)
  506    format(20x,'* GOES AVAILABLE,       OHC PROXY USED      *' )
      else
         write(lush,507)
  507    format(20x,'* GOES PROXY USED,      OHC PROXY USED      *' )
      endif
...
c 518 format('MO FLX (M/S/D)',21(2x,a4))
  ...
  537 format('700-850 TADV  ',21(2x,a4))
...
c        Calculate intensity change contributions
     ...
     +                     vart(12,kk)*coef(12,kk)*ysig(kk)
            ...
            dvvar(13,kk) = vart(24,kk)*coef(24,kk)*ysig(kk) 
     ...
     +                     vart( 8,kk)*coef( 8,kk)*ysig(kk)
            dvvar(18,kk) = vart(21,kk)*coef(21,kk)*ysig(kk)
c
c           do n=1,nvar
...
c
      if (ibasin .eq. 1) then
         call rapidga_2011(perri,shdcri,d200ri,potri,rhlori,
     +                sbtri,pcri30,rhcri,sname,natcf8,ismon,isday,isyr,
     +                istime,lush)
      else
         call rapidge_2011(perri,shdcri,d200ri,potri,rhlori,
     +                sbtri,pcri30,rhcri,sname,natcf8,ismon,isday,isyr,
     +                istime,lush)
      endif
c
...
c
  900 continue
      write(lulg,*) ' Error during file open'
...
c
      dimension idmon(12)
      data idmon /0,31,59,90,120,151,181,212,243,273,304,334/
c
...
c
      ...
      dimension ishdc(0:mft),irhcn(0:mft),itadv(0:mft)
...
c     ++ Local variables for variables derived from common variables
      ...
      dimension shrl(0:mft),sstl(0:mft),rhcn(0:mft),tadv(0:mft)
...
c     **** Specify basin-specific variables ****
      if (ibasin .eq. 1) then
         if (ioper .eq. 1) then
            fncol=trim( coef_location )//'shipl11_coef_atlc.dat'
         else
            fncol='shipl11_coef_atlc.dat'
         endif
      else
         if (ioper .eq. 1) then
            fncol=trim( coef_location )//'shipl11_coef_epac.dat'
         else
            fncol='shipl11_coef_epac.dat'
         endif
      endif
c
...
c
c     Copy rlat,rlon,vsstl to temporary arrays for the lgeim call
      do k=0,mft
         tlat(k+1) = rlat(k)
         tlon(k+1) = -rlon(k)
         vmpi(k+1) = vsstl(k)
...
c
      ...
      subroutine tspdcal(tlat,tlon,ttime,mft,rmiss,cx,cy,cmag)
...
c     Missing lat/lon points (equal to rmiss) are accounted for. 
c
c     This routine assumes lat is in degree N
c                          lon is in degree W positive
c     
      dimension tlat(-2:mft),tlon(-2:mft),ttime(-2:mft)
      dimension cx(-2:mft),cy(-2:mft),cmag(-2:mft)
c
c     Local variables
      parameter(itmx=500)
      dimension ttlat(-3:itmx),ttlon(-3:itmx)
c
...
c
      do 99 k=-2,mft
         ...
         i0=0
         ip=0
         if (ttlat(k-1) .lt. rmiss) im=1
         if (ttlat(k  ) .lt. rmiss) i0=1
         if (ttlat(k+1) .lt. rmiss) ip=1
c
...
*
* Jim Kossin
* Determines endian-ness of host machine. Returns index 1 for Little
* Endian, index 4 for Big Endian. The index specifies least significant
* byte position of 4-byte word.
*
      integer i,j,idum
      byte word(4)
...
c-----7---------------------------------------------------------------72
      ...
      if (charmon.eq.'JAN') then
           intmon=1
	   goto 888
      endif
      
      if (charmon.eq.'FEB') then
           intmon=2
	   goto 888
      endif
      
      if (charmon.eq.'MAR') then
           intmon=3
	   goto 888
      endif
      
      if (charmon.eq.'APR') then
           intmon=4
	   goto 888
      endif
      
      if (charmon.eq.'MAY') then
           intmon=5
	   goto 888
      endif
      
      if (charmon.eq.'JUN') then
           intmon=6
	   goto 888
      endif
      
      if (charmon.eq.'JUL') then
           intmon=7
	   goto 888
      endif
      
      if (charmon.eq.'AUG') then
           intmon=8
	   goto 888
      endif
      
      if (charmon.eq.'SEP') then
           intmon=9
	   goto 888
      endif
      
      if (charmon.eq.'OCT') then
           intmon=10
	   goto 888
      endif
      
      if (charmon.eq.'NOV') then
           intmon=11
	   goto 888
      endif
      
      if (charmon.eq.'DEC') then
           intmon=12
	   goto 888
      endif
