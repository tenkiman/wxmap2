      ...
      use module_control,only : include_path

      integer ioper

c         -- works everywhere with access to /data/amb/users -- put in namelist
c
c      character(len=75), parameter :: include_path='/data/amb/users/fiorino/w21/dat/geog/coastlines'
c      character(len=75), parameter :: include_path='/w21/dat/geog/coastlines'
...
C      REAL*8 CISL(NCOAST)
     ...
     2       DWRK(NISLSOB), BWRK(NISLSOB)
      INTEGER*4 NPT(NCOAST), LT(NCOAST)
      CHARACTER CISL(NCOAST)*8
      LOGICAL*1 FLAG1S, LPR,verb

      character*75 shland_file
...
c
      ...
      if(verb) print*,'shland_file: ',shland_file
...
C
      ...
      if(verb) print*,'nnnnnnnnnn ncons',ncons,cisl(1),lt(1)
...
C
      DO 105 I = 1,NCONS
        ...
        if(verb) print*,'n1 ',xcons(i,1),xcons(i,2)
  207    FORMAT(F7.1,F6.1)
  105 CONTINUE
C
...
C
  ...
  200 NISLS = NISLS + 1
      ...
      READ(5,209,END=407,err=407) NPT(N), CISL(N), LT(N)
  209 FORMAT(I3,1X,A8,1X,I1)
      if(verb) print*,'nnnnnnnnnnnnn ',n,npt(n),cisl(n),lt(n)

      NPTS = NPT(N)

      DO 109 I = 1,NPTS
        READ(5,207,END=995) XISLS(N,I,1), XISLS(N,I,2)
        if(verb) print*,'islands: ',n,i,xisls(n,i,1),xisls(n,i,2)
  109 CONTINUE
C
C     COPY INITIAL POINT INTO NPTS+1 ARRAY SPACE (WILL BE NEEDED TO 
C     COMPUTE DISTANCE AND BEARING FROM LAST POINT IN CLOSED LOOP TO
