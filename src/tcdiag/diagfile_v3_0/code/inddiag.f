...
c            lsdiags.f
c
c     Last Modified: 05/15/2015, version 3.0
c        -altered processing of latitude and longitude arrays
c     Modified:      12/31/2012, version 2.1
...
c 
      ...
      real :: latstartp, latendp, latintp
      real :: latstartn, latendn, latintn
      real :: lonstartp, lonendp, lonintp
      real :: lonstartn, lonendn, lonintn
...
c 
      !debugging test, write out each input
c      write(*,*) nxp, nyp, latstartp, latendp, latintp
c      write(*,*) lonstartp, lonendp, lonintp, npointsp
c 
      ...
      rlon=rmiss
      do n=1,nyp
         if (latstartp .le. latendp) then
            rlat(n)=latstartp+latintp*(n-1)
         else
            rlat(n)=latstartp-latintp*(n-1)
         endif
      enddo
      !debugging test, write out latitude
c      write(*,*) "rlat(1)=", rlat(1), " rlat(nyp)=", rlat(nyp)
c      write(*,*) "nyp=", nyp
      do n=1,nxp
         if (lonstartp .le. lonendp) then
            rlon(n)=lonstartp+lonintp*(n-1)
         else
            rlon(n)=lonstartp-lonintp*(n-1)
         endif
      enddo
      !debugging test, write out longitude 
c      write(*,*) "rlon(1)=", rlon(1), " rlon(nxp)=", rlon(nxp)
c      write(*,*) "nxp=", nxp
c 
...
c 
      !debugging test, write out each input
c      write(*,*) nxn, nyn, latstartn, latendn, latintn
c      write(*,*) lonstartn, lonendn, lonintn, npointsn
c 
      !allocate and calculate rlat, rlon
         ...
         rlonn=rmiss
         do n=1,nyn
            if (latstartn .le. latendn) then
               rlatn(n)=latstartn+latintn*(n-1)
            else
               rlatn(n)=latstartn-latintn*(n-1)
            endif
         enddo
         !debugging test, write out latitude
c         write(*,*) "rlatn(1)=", rlatn(1), " rlatn(nyn)=", rlatn(nyn)
c         write(*,*) "nyn=", nyn
         do n=1,nxn
            if (lonstartn .le. lonendn) then
               rlonn(n)=lonstartn+lonintn*(n-1)
            else
               rlonn(n)=lonstartn-lonintn*(n-1)
            endif
         enddo
         !debugging test, write out longitude 
c         write(*,*) "rlonn(1)=", rlonn(1), " rlonn(nxn)=", rlonn(nxn)
c         write(*,*) "nxn=", nxn
c 
