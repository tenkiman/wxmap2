...
c
      ierr = 0
      clon = xgrd*dlon + blon - dlon
      if(verb) print*,'xxxyyylllll xxxx',xgrd,dlon,blon,dlon,ierr
      if (clon.lt.0.0 .or. clon.gt. 360.0) then
        write (33,*) 'cxytll, x-grid error, x-grid = ',xgrd
        ierr = -1
        stop 'cxytll lon'
      endif
      clat = ygrd*dlat + blat - dlat
      if(verb) print*,'xxxyyylllll yyyyy',ygrd,dlat,blat,dlat
