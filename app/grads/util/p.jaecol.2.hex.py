...
for card in cards:
    if(mf.find(card,'set rgb') and not(card[0] == '*' or card[0] == '#') ):
        card=card.replace("'","")
        tt=card.split()
        ndx=int(tt[2])
        red=int(tt[3])
        green=int(tt[4])
        blue=int(tt[5])
        hcolor="#%s%s%s"%(i2h(red),i2h(green),i2h(blue))
        pycard=''' %d:'%s',  # %03d %03d %03d'''%(ndx,hcolor,red,green,blue)
        pylist=pylist+pycard+'\n'
        print card,ndx,i2h(red),green,blue,' hcolor: ',hcolor
