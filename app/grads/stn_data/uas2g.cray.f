...
C         cray:
C
C         cf77 uas2g.f -o uas2g
C         uas2g
C         stnmap -i uas.ctl
C         grads -lc "open uas.ctl"
C         
C         unix:
C
...
C         cray:
C
C         uas.cray.obs    GrADS binary format
C         uas.cray.smp    station map file (16 bytes on 32-bit machines
C         uas.cary.ctl    GrADS data descriptor file
C         
C---      np - max # of levels
C
      parameter(np=500)
...
C
      card='assign -s unblocked -a uas.cray.obs fort.12'
      call assign(card)
