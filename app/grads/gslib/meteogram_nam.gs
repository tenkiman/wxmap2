...
#
  ...
  mmddhh=substr(date,5,6)
  _dataset = 'eta'mmddhh
  dpath=_baseurl%_dataset
  fc=ofile(dpath,'sdf')
else
  dpath='/tmp/nomads.nam.ctl'
  fc=ofile(dpath,'xdf')
endif

if(fc = 0)
  print 'WWWWWWWW data for dtg: 'date' not available....'
  'quit'
else
  print 'success opening: 'dpath' onward ...'
endif
...
_tdim = _time1' '_time2
_tincr = subwrd(_tdef,5)
_tdef = 'tdef '_tsize' linear '_time1' '_tincr
print 'qqqq '_tsize' '_tincr
if(_tincr = '180mn') ; _dhr=3 ; _tbucket=12 ; endif
if(_tincr = '60mn')  ; _dhr=1 ; _tbucket=12 ; endif
if(_tincr = '3hr')   ; _dhr=3 ; _tbucket=12 ; endif
if(_tincr = '6hr')   ; _dhr=6 ; _tbucket=12 ; endif
_nhr=(_tsize-1)*_dhr

#
# calc file name to include lenght of the forecast
#
if(pdir='') ; pdir='/tmp' ; endif

print 'qqqqqqqqqqqqqqqqqqqqqqq bucket '_tbucket' 'ntip' '_dhr' 'np' '_tsize

if(usecola=1)
pbase=pdir'/meteogram.eta.'name'.'date'.tau.0.'_nhr'.'_dhr'.cola'
else
pbase=pdir'/meteogram.eta.'name'.'date'.tau.0.'_nhr'.'_dhr'.ncep'
endif
...
while (n >= 2)
  ...
  n = n - 1
endwhile
xincr = (8.15 - 1.2)/_tsize           ;* size of one time step
...
#
# if point is above 850 set toto to  barbase
#
'toto=const(toto,40,-u)'
'set axlim 11 69'
'set yaxis 11 69 10'
'set ccolor 8'
'set t '_t1-0.5' '_t2+0.5
...
'set strsiz 0.12'
#
# convert to 2 digit precision
#
print 'hhhhbbb 'hilon' 'hilat
chilon=math_format('%7.2f',hilon)
chilat=math_format('%6.2f',hilat)
print 'hhhhccc 'chilon' 'chilat

#
#mmmmmmmmmm main title
#
# Draw Labels at the top of the page

'set line 0'
'draw recf 0 10.48 8.5 11.0'

'set line 0'
'draw recf 0 0 8.5 0.10'

label = 'ETA 'date' 0-'_nhr' h Forecast for ('
if (hilon < 0)  ; label = label%math_format('%7.2f',hilon*(-1.0))'W, ' ; endif
if (hilon >= 0) ; label = label%chilon'E, ' ; endif
#if (hilat < 0)  ; label = label%chilat*(-1.0)'S)'; endif
if (hilat < 0)  ; label = label%math_format('%6.2f',hilat*(-1.0))'S)'; endif
if (hilat >= 0) ; label = label%chilat'N)' ; endif
'draw string 8.15 10.80 'label
...
'disable print'
'!gxeps -c 'gmpath
'printim 'pngpath' x'xsiz' y'ysiz' white'

print 'PPPPPPPPPPPPPPP: 'epspath
