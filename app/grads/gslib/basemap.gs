...
if (args='') 
  say 'Usage: basemap L(and)/O(cean) <fill_color> <outline_color> <L(owres)/M(res)/H(ires)> <0(no frame)/1(frame)>'
  return
else 
  type    = subwrd(args,1)
  color   = subwrd(args,2)
  ...
  if (fraeon = '') ;           frameon=1 ; endif
endif 

if(_polydir = '' | _polydir = '_polydir')
  _polydir='/home/fiorino/grads/gslib'
  _polydir='/pcmdi/chico_dat/wxmap2/grads/gslib'
endif

* Set the polygon data files
if (type = 'L' | type = 'l') 
*  if (lowres); file = _polydir'/lpoly_US.asc'; endif
  if (lowres); file = _polydir'/lpoly_lowres.asc'; endif
  if (mres)  ; file = _polydir'/lpoly_mres.asc'  ; endif
  if (hires) ; file = _polydir'/lpoly_hires.asc' ; endif
endif
if (type = 'O' | type = 'o') 
  if (lowres); file = _polydir'/opoly_lowres.asc'; endif
  if (mres)  ; file = _polydir'/opoly_mres.asc'  ; endif
  if (hires) ; file = _polydir'/opoly_hires.asc' ; endif
endif
