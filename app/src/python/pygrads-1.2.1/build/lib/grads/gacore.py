...
* changed __init__ to make subclassing simpler

* added start up options to support .py lats (RcCheck) and
  output of grads commands to a log file
...
class GaCore(GrADSObject):
    ...
    *GanNum* and *GaLab*.
    ...
    def initGaCore(self):
        ...
        (mf) RcCheck   turn off ALL check of return code (rc) from GrADS commands
        (mf) doLogger  output grads commands to  logpath="/tmp/gacore.%s.log"%(curpid); 
                       curpid=os.getpid()
...
#       ------------------------
        cmdline = self.Bin + ' -u'
        if self.Window != True: cmdline = cmdline + ' -b'
        if self.Port   == True: cmdline = cmdline + ' -p'
