...
###    jetInvDir='/lfs2/projects/fim/fiorino/w21/dat/nwp2/DSs'
    ...
    def gettaus(self,files,warn=1,verb=0):
        taus=[]
        for file in files:
            if(verb): print('ffff ',file)
            # -- special case for gfs goes imagery
            #
            if(mf.find(file,'goes')):
                ttau=file.split('.')[-3]
            else:
                ttau=file.split('.')[-2]
            
            if(ttau[0] != 'f' or len(ttau) != 4):
                if(warn): print('WWW(nwp.Nwp2Data.gettaus): --- bad file name for getting tau: ',file)
                taus.append(0)
                return(taus)
            tau=int(ttau[1:])
            taus.append(tau)
    ...
    def invLocalOld(self,overrideKey1=None,verb=1):
        ...
        for s1 in list(s1dirs.keys()):
            s2dirs=self.getsubdirs(s1dirs[s1])

            for s2dir in list(s2dirs.keys()):
                MF.sTimer(s2dir)
                taus={}

                s3dirs=self.getsubdirs(s2dirs[s2dir])
                sdir=s2dirs[s2dir]
                dtgs=list(s3dirs.keys())
    ...
    def getDtgDirs(self,model):
        ...
        kk=list(self.center_model_dtgs.keys())
        for k in kk:
            if (model == k[1]):
    ...
    def setLocalMssPaths(self,n2hash,tbdir,model,dtg):
        ...
        for dmodel in list(self.dmodels.keys()):
    ...
    def setLocalHpssPaths(self,n2hash,tbdir,model,dtg):
        ...
        try:
            dmodel=list(self.dmodels.keys())[list(self.dmodels.values()).index(model)]
        except:
    ...
    def lsAllMssDtgs(self,model,rmodel=None):
        ...
        if(hasattr(self,'AllMssModDtgs')):
            ...
            models=list(self.AllMssModDtgs.keys())
...
####    jetbdir='/lfs2/projects/fim/fiorino/rtfim/dat'
    ...
    def lsAllLocalDtgs(self,fimrun,rmodel=None,verb=0):
        ...
        fimruns=list(self.localModelDtgs.keys())
    ...
    def getlocalModels(self,tdtgs):
        ...
        fimruns=[]
        models=[]
        frs=list(self.localModelDtgs.keys())
        frs=MF.uniq(frs)
