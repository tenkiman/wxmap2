...
def TcstructDataCards(dtg,stmdata,stm3tofull,otdir,ogdir,osdir,odir,verb=0):
    ...
    for stm in stmids:
        ...
        (clat,clon)=TC.Rlatlon2Clatlon(stmrlat,stmrlon)
...
def JtwcLatLonBounds(cdirjtwc,cdirnhc,fstm,stmdata,stmmotion,dtg):
    ...
    if(NewLatlonBoundsScheme==1):
        ...
        dt=24.0
        dt=48.0
        (rlatoffset,rlonoffset)=TC.rumltlg(stmdir,stmspd,dt,stmrlat,stmrlon)
    ...
    basinid=TC.Basin1toBasin2[stm[2:]]
...
def ParseCarqStorms(dtg,stmidset='all'):
    ...
    for stmid in stmids:
        ...
        if(stmdtgm12 != 'NA'):
            ...
            (rlat1,rlon1)=TC.Clatlon2Rlatlon(stmclat,stmclon)
            (rlat0,rlon0)=TC.Clatlon2Rlatlon(stmclatm12,stmclonm12)
...
def LoadStmDataBtops(n,stmids,stmdata,stmmotion,stmids1,stmdata1,stmmotion1):
    ...
    #
    (clat,clon)=TC.Rlatlon2Clatlon(rlat,rlon)
    posit[1]=" %s"%(clat)
    posit[2]=" %s"%(clon)
...
def Carq2Ngtrp(dtg,stmids,stmdata,stmmotion,ngtrppath):
    ...
    for stmid in stmids:
        ...
        (clat,clon,ilat,ilon,hemns,hemew)=TC.Rlatlon2ClatlonFull(rlat,rlon)
...
#nnnn  54 K
    ...
    for mdcard in btops:
        ...
        btc=TC.ParseMdeckCard2Btcs(mdcard)
...
def ParseNgtrkNgtrpDiagCards(ngtrkngtrppath,ngtrkdiagmfpath,verb=0):
    ...
    while(n < ncards):
        ...
        (tcrlat,tcrlon)=TC.Clatlon2Rlatlon(tt[0],tt[1])
    ...
    for stmid in stmids:
        ...
        for itau in stmtaus[stmid]:
            ...
            try:
                ...
                (clat,clon,ilat,ilon,hemns,hemew)=TC.Rlatlon2ClatlonFull(rlatfc,rlonfc)
...
def SetModels(dtg,modelopt):
    ...
    if(modelopt == 'all'):
        if(hh == '06' or hh == '18'):
            models=TC.TcanalModelsoff
        else:
            models=TC.TcanalModels
            
    elif(w2.IsModel2(modelopt)):
        models.append(modelopt)

    else:
        for tcmodel in TC.TcanalModels:
            if(tcmodel == modelopt):
                models.append(modelopt)
