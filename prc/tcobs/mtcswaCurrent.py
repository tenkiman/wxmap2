...
class MtcswaObs:
    ...
    Vars = ['u','v','w','uso','vso','wso','uvr','vvr','uvt','vvt']
    ...
    VarDesc={
             ...
             'uvr':'u comp radial',
             'vvr':'v comp radial',
             'uvt':'u comp tangential',
             'vvt':'v comp tangential'
            }
    VarUnits={
              'u':'[m/s]',
              'v':'[m/s]',
    ...
    def CtlFile(self,ctlPath,smpPath,outPath,dtg,datatype):
        ...
        for v in self.Vars:
            if ( datatype == self.DataTypes[0] or datatype == self.DataTypes[1]):
...
#
    ...
    def WriteAllOutput(self,basemeta,stn,boolean):
        ...
        if ( typ == self.DataTypes[0] or typ == self.DataTypes[1] ):
            stnrec = struct.pack('%df'%11,psr,uwd,vwd,wsp,uso,vso,wso,uvr,vvr,uvt,vvt)
        #elif ( typ == self.DataTypes[1] or typ == self.DataTypes[2] ):
        #elif (  ):
        #    stnrec = struct.pack('%df'%4,psr,uwd,vwd,wsp)
        #elif ( typ == self.DataTypes[3] ):
