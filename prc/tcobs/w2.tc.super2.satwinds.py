...
"""%(outFile,sFile,self.Undef,gtime,len(vars))
    ...
    def SuperObsAveraging(self,qcmeta):
        ...
        for stn in stns:
            ...
            for inc in range(len(lats_0)):
                ...
                if ( radius <= (self.deltaR*0.5)):
                    try:
                        winds[type,prslev][lats_0[inc],lons_0[inc]].append(wdspd)
                        ucomp[type,prslev][lats_0[inc],lons_0[inc]].append(uwind)
                        vcomp[type,prslev][lats_0[inc],lons_0[inc]].append(vwind)
                        temps[type,prslev][lats_0[inc],lons_0[inc]].append(temp)
                        hgt[type,prslev][lats_0[inc],lons_0[inc]].append(height)
                    except:
                        try:
                            winds[type,prslev][lats_0[inc],lons_0[inc]] = []
                            winds[type,prslev][lats_0[inc],lons_0[inc]].append(wdspd)
                            ucomp[type,prslev][lats_0[inc],lons_0[inc]] = []
                            ucomp[type,prslev][lats_0[inc],lons_0[inc]].append(uwind)
                            vcomp[type,prslev][lats_0[inc],lons_0[inc]] = []
                            vcomp[type,prslev][lats_0[inc],lons_0[inc]].append(vwind)
                            temps[type,prslev][lats_0[inc],lons_0[inc]] = []
                            temps[type,prslev][lats_0[inc],lons_0[inc]].append(temp)
                            hgt[type,prslev][lats_0[inc],lons_0[inc]] = []
                            hgt[type,prslev][lats_0[inc],lons_0[inc]].append(height)
                        except:
                            winds[type,prslev] = {}
                            winds[type,prslev][lats_0[inc],lons_0[inc]] = []
        ...
        for typeelev in typlev:
            ltln = winds[typeelev].keys()      
            ltln.sort()
            for latlon in ltln:
                avewind = numpy.mean(winds[typeelev][latlon])
                aveu = numpy.mean(ucomp[typeelev][latlon])
                avev = numpy.mean(vcomp[typeelev][latlon])
                avet = numpy.mean(temps[typeelev][latlon])
                avez = numpy.mean(hgt[typeelev][latlon])
                
                meta = (latlon[0], latlon[1], typeelev[1], aveu, avev, avewind, avet, avez, typeelev[0])
