...
class SatWinds:
    ...
    DataTypes = ['AMSU','IRWD','CD','ASCT','WV','SCAT','SSMI'] 
    ...
    def __init__(self,dtg,storm,file,cLat,cLon):
        __inputPath  = '%s%s/%s/%s%s.obs'%(self.Inputdir,dtg[0:4],storm,dtg,file)
        print __inputPath
        self.outputPath = {}
        self.ctlPath    = {}
        self.smpPath    = {}
    ...
    def CtlFile(self,ctlPath,smpPath,outPath,dtg,datatype):
        ...
        for v in self.Vars:
            ...
            elif ( (datatype == self.DataTypes[1] or datatype == self.DataTypes[2] or datatype == self.DataTypes[4] or datatype == self.DataTypes[5]) and v != self.Vars[3] and v != self.Vars[4] ):
                desc='%s %s'%(self.VarDesc[v],self.VarUnits[v])
                vars.append('%-7s 1 0 %s'%(v,desc))
...
#
    ...
    def WriteOutput(self,basemeta,stn):
        ...
        elif ( typ == self.DataTypes[1] or typ == self.DataTypes[2] or type == self.DataTypes[4] ):
            stnrec = struct.pack('%df'%4,psr,uwd,vwd,wsp)
        elif ( typ == self.DataTypes[3] or typ == self.DataTypes[5] or typ == self.DataTypes[6]):
...
if(__name__ == '__main__'):
    ...
    satwinds = SatWinds('2009082906','94e','_EP9409',14.5,-101.9)
