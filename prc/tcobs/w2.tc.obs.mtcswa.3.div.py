...
class Mtcswa(MFbase):
    ...
    def draw(self,dtg,storm,lat,lon,subtype, drawBufr, pressure,vmax,mprs):
        ...
        for lev in pressure:
            ...
            cmd += "set clevs -10 -7.5 -5 -2.5 2.5 5 7.5 10\n"
            #cmd += "set ccols 5 13 3 10 0 0 7 8 2 6\n"
            cmd += "set ccols 14 4 11 5 0 7 8 2 6\n"
            #cmd += "set rbrange -10 10\nset cint 2.5\n"

            cmd += "d hdivg(oacres(re2(u.2(t=1),0.25),u.1),oacres(re2(v.2(t=1),0.25),v.1))*1e5\nset gxout contour\nset clevs -10 -7.5 -5 -2.5 2.5 5 7.5 10\nset ccolor 1\n"
    ...
    draw title  %s - IRWD - %s\Super Obs [kts]/H Divg [10`a-5`n s`a-1`n] 
    ...
    d uso;vso
    draw wxsym 41 5.5 4.2 0.8 1 50
    draw title  %s - IRWD - %s\Super Ob Radial(red) & Tangential Winds(green)
    draw string 0.5 5.6 clat: %4.1f
    draw string 0.5 5.3 clon: %5.1f
    draw string 0.5 5.0 vmax: %d kts
    draw string 0.5 4.7 pmin: %d mb
    draw string 0.5 4.4 plev: %d mb
    gxyat /w3/rapb/slocum/tcobs/04e.05e.2010/vrvt/04e/%s.%s.winds.vrvt.png
    c

    """%(storm,dtg,lat,clon,vmax,mprs,pressure[0],dtg,storm,storm,dtg,lat,clon,vmax,mprs,pressure[0],dtg,storm)
