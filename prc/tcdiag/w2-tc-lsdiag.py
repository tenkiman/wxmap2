...
class MyCmdLine(CmdLine):
    ...
    def __init__(self,argv=sys.argv):
        ...
        self.options={
            ...
            'bypassRunChk':     ['y',1,0,'do NOT bypassRunChk track...'],
    ...
    for model in models:
        ...
        if(doInv):
            ...
            if(stmopt == '09w.12'):
                ...
                tGi=TcdiagInv(tG,dtgopt=dtgopt,invtag='11w.2013',ndayback=ndayback,tstmid='11w.2013',keepmodels=keepmodels)
            elif(stmopt == '07p.14'):
                tGi=TcdiagInv(tG,dtgopt=dtgopt,invtag='07p.2014',ndayback=ndayback,tstmid='07p.2014',keepmodels=keepmodels)
            else:
        ...
        #
        if(len(tG.stmids) == 0):
            print """WWW(%s) NNNNN no TCs for dtg: %s model: %s"""%(pyfile,dtg,model)
            #didIt=0
            continue
        if(len(tG.adstm2ids) == 0):
            print """WWW(%s) TTTT trkSource: '%s' either NOT run or FAILED to run for dtg: %s model: %s"""%(pyfile,trkSource,dtg,model)
            #didIt=0
            continue
        ...
        #
        chkOquiet=0
        (rc,todoStmids)=tG.chKOutput(dtg,tbdir,dstmids,aidSource=aidSource,quiet=chkOquiet)
        ...
        #
        if(dolsDiag or dols): 
            if(dolsDiag or dols): rc=lsDiagProc()
            continue
        ...
        # -- load inventory
        #
        (rc,todoStmids)=tG.chKOutput(dtg,tbdir,dstmids,aidSource=aidSource,quiet=1)
