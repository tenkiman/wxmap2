...
# -- 20210713 -- meld w2-tc-lsdiag-tau0.py into -sbt.py
#
...
class MyCmdLine(CmdLine):
    ...
    def __init__(self,argv=sys.argv):
        ...
        self.options={
            ...
            'docleanDpaths':    ['K',1,0,'do NOT clean off dpaths for current dtg'],
...
if(doclean):
    ...
    for ddtg in alldtgs:
        ...
        if(testdiff <= 0.0):
            datfiles=glob.glob("%s/%s/*/*.dat"%(tbdir,ddtg))
...
if(ropt != 'norun'):
    ...
    #
    MF.sTimer('TcData:%s'%(dtgopt))
    tD=TcData(dtgopt=dtgopt)
    MF.dTimer('TcData:%s'%(dtgopt))
...
for dtg in dtgs:
    ...
    for model in models:
        ...
        for stmid in dstmids:
            ...
            if(rc == 0):
                print 'no joy in lsdiag.x...sayounara...'
                sys.exit()
