...
#
# dtg of vertical line to indicate current date
#
y1=substr(dtg1,1,4)
cy1=substr(_curdtg,1,4)
mm1=substr(dtg1,5,2)
mm2=substr(dtg2,5,2)
cmm=substr(_curdtg,5,2)

cym1=substr(dtg1,1,4)%substr(_curdtg,5,6)
cym2=substr(dtg2,1,4)%substr(_curdtg,5,6)

if(cym1 >= dtg1 & cym1 <= dtg2)
  dldtg=cym1
else
  dldtg=cym2
endif

#print 'dddddddddddddd 'cym1' 'cym2' 'dtg1' 'dtg2' 'dldtg

rc=setup(0)
dfile='./dum.tc.activity.ctl'
rc=ofile(dfile)
...
_ic.0=0
_ic.1=7  ; _is.1='TD (<35)'  ;
_ic.2=3  ; _is.2='TS (35->65)' ;
_ic.3=2  ; _is.3='TY (>65<100)'  ;
_ic.4=5  ; _is.4='TY (>100)'  ;
_ic.5=1  ; _is.5='STY (>=130)'  ;

#rc=setbackground(1)
rc=setgrads()

'set xyrev on'
'set parea 1.25 10.5 1.20 7.75'
...
'set yflip on'
'set ylopts 1 8 0.15'
'set ylabs || LANT || EPAC || WPAC || NIO || SIO || SWPAC ||'
'set ylabs || LANT || EPAC || WPAC || NIO || SIO || SWPAC ||'

#'set x 1 8'
#'set ylopts 0.5 6.5 0.15'
#'set ylabs LANT || EPAC || WPAC || NIO || SIO || SWPAC ||'
...
while(z<=_nb)
  'q w2xy 'time1' 'z
  _zx.z=subwrd(result,3)-1.15
  _zy.z=subwrd(result,6)-0.375
  rc=boxtbl(_zx.z,_zy.z,_ncnt.z)
  z=z+1
endwhile
...
t1='Tropical Cyclones by Basin for: 'ymd1'-'ymd2
t2='bars colorized by intensity, multiply TCs below main line as of `2'_curdtg
ttlscl=1.20
t1col=1
t2col=4
...
yln=0.60
dyl=xlsz
rc=boxlgd(ni,dxoff,xlsft,xlsz,yln,dyl)

rc=rdngtrp()

gname=gname'.BT.final'
gpath=gdir'/'gname
...
#
# draw line to indicate current date
#
rc=drawdline(dldtg)
...
function drawdline(dldtg)

xoff=0.05
gtime=dtg2gtime(dldtg)

rc=plotdims()
'q w2xy 'gtime' '_ybplot
xdline=subwrd(result,3)
xdline=xdline+xoff
print 'xdline 'xdline
'set line 4 0 8'
'draw line 'xdline' '_ybplot' 'xdline' '_ytplot
...
logfile='/tmp/plot.log_'_script'_.txt'
...
sumfile='ngtrp.tc.activity.summary.txt'

print 'summm 'rc' 'card' 'iok
...
while(i<=_nb)
  ...
  card=sublin(rc,2)
  iok=sublin(rc,1)
  _ncnt.i=substr(card,2,25)
#  if(iok != 0 & i < _nb) 
#    say 'EEEE error reading ngtrp summary file!!!'
#    say 'BYE'
#    'quit'
#  endif
  print 'NNNN  '_ncnt.i
...
runfile='/era/data/erf/pcmdi/fiorino/nwp/wxmap/dat/tc/ngtrp_all'
runfile='/era/data/erf/pcmdi/fiorino/nwp/wxmap/dat/tc/ngtrp_1987'
runfile='ngtrp.txt'
runfile='/era/data/erf/pcmdi/fiorino/nwp/wxmap/dat/tc/ngtrp_1998-1999'
runfile='/era/data/erf/pcmdi/fiorino/nwp/wxmap/dat/tc/ngtrp_1999'
runfile='/pcmdi/typhoon_d1/nwp/wxmap/dat/tc/ngtrp_1999'
runfile='ngtrp.tc.activity.txt'
sumfile='ngtrp.tc.activity.summary.txt'
...
if(iok=0)
  ...
  ctime=dtghcur(dtg)
####  print ' '
####  if(ns>=2) ; print ' 'ctime' NNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNN' ; endif
  if(ns>0)
    n=1
    while(n<=ns) 
      rc=read(runfile)
      card=sublin(rc,2)
      iok=sublin(rc,1)
      if(iok=2) ; return ; endif
      vmax=subwrd(card,3)*1
      snum=subwrd(card,4)
      basin=subwrd(card,5)
      sid.n=snum%basin
      xb.n=basin2x(basin)
      'q w2xy 'ctime' 'xb.n
      xs.n=subwrd(result,3)
      ys.n=subwrd(result,6)
      mt.n=5
      ms.n=0.2
      mc.n=vmax2c(vmax)
#####      print sid.n'  vmax = 'vmax' 'ctime' 'mc.n
      n=n+1
    endwhile
    ...
    while(n<=ns)
      ...
      if(xb.n = xb.nm1)
        ys.n=ys.nm1-0.25
#       print 'NNNNNNNNNNNNNNNNNNNNNNNNNNNNNN ns= 'ns'  'sid.n' 'xb.n' 'xb.nm1' 'ys.n' 'ys.nm1
     endif
      n=n+1
    endwhile
...
###      'draw mark 'mt.n' 'xs.n' 'ys.n' 'ms.n

     if(x1>=_xlplot & x2 <=_xrplot)
...
function vmax2c(vmax)

if(vmax < 35) ; c=_ic.1; endif
if(vmax >= 35 & vmax < 65) ; c=_ic.2 ; endif
if(vmax >= 65 & vmax < 100) ; c=_ic.3 ; endif
if(vmax >= 100 & vmax < 130) ; c=_ic.4 ; endif
if(vmax >= 130) ; c=_ic.5 ; endif
return(c)


function basin2x(basin)

if(basin='L' | basin = 'Q') ; x=1; endif
if(basin='E' | basin='C') ; x=2; endif
if(basin='W') ; x=3; endif
if(basin='B' | basin='A' |  basin='I') ; x=4; endif
if(basin='S') ; x=5; endif
if(basin='P') ; x=6; endif
if(basin='T') ; x=1; endif
...
#----------------------------------------------------------------------
#
#  plot 3x2 box with numbers/text
#
#----------------------------------------------------------------------


function boxtbl(x0,y0,text)
...
function boxlgd(nm,dxoff,xlsft,xlsz,yln,dyl)
xlen=_pagex-2*dxoff+0.5
ylg=yln+dyl
dx=xlen/nm
...
while(j<=nm)
  ...
  y1=yln-dyl
  y2=yln+dyl
  'draw recf 'xb' 'y1' 'xe' 'y2

  'set string 1 l 5'
  'set strsiz 0.125'
  'draw string  'xm' 'yln' '_is.j
  j=j+1
endwhile
