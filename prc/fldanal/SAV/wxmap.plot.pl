...
$wxdir=$ENV{"W2_PRC_DIR"};
require("$wxdir/fldanal/wxmap.env.pl");
wxmap_env();
...
#
# set tcopt here because it's needed by tc_posits in wxmap.env.pl
#
$tcopt="ops";
...
if($narg >= 3) {
  ...
  print "\n";
  print "   [specopt] : [''] | npmoc.ldm (for ngp)\n";
...
if($specopt eq "npmoc.ldm")  {
    $specoptgrads="npmocldm";
} else {
    $specoptgrads=$spectopt;
}

#
# set up the .gs file by combining a base + .gsf with tc posits
#

$gsbase="g.wxmap.base.gs";
$gstc="tcposit.noload.gsf";
$gsfinal="g.w1.$model.$dtg.gs";

system("cat $gsbase > $gsfinal");

$tcgsfdir=$WXMAP{'WXMAP_TC_STRUCT_DAT_DIR'};
$tcgsfpath="$tcgsfdir/$dtg/trk/ngtrk.tcbtft.$model.$dtg.gsf";
if ( (-f $tcgsfpath) && (-s $tcgsfpath) > 0) {
    print "TCTCTCTCTC got tcstruct .gsf\n";
    $gstc=$tcgsfpath;
}
...
#
system("cat $gstc >> $gsfinal");

if($nplot > 0) {
  $cmd="$gradsexe $gmode \"$gsfinal $wxifile $specoptgrads\" -g ${xs}x${ys}";
  print"QQQ $cmd\n";
  system($cmd) if(! $test);
}
