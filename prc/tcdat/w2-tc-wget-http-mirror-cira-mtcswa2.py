...
af=w2.TcMtcswa2FtpServer
al=w2.TcMtcswa2FtpLogin
ap=w2.TcMtcswa2FtpPasswd
sbdir=w2.TcMtcswa2FtpDatDir
tbdir=w2.TcMtcswa2DatDir
...
for dtg in dtgs:
    ...
    for stmid in stmids:
        ...
        #
        logpath="%s/db.wget2.%s.%s.txt"%(tdir,stmid,dtg)
        ...
        #
        incdir=sdir.replace('NC','')
        wgetopt="-m -np -nH --cut-dirs=4 -nv -T 60 -t 1 -A *%s* -R *log*"%\
            (stm2id[0:4])
