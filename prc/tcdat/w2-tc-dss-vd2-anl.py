...
if(reloadFromPyp):
    ...
    # -- set doprint opt if ls
    doprintOstat=1
    if(ptype == 'ls'): doprintOstat=0
    ...
    if(ptype == 'pod' or ptype == 'pof'):
        ...
        (filtPof,filt0012,filt0618,filt00,filt12,filtVmax35)=getPodFiltOpts(ptype,filterOpts)
        # -- counts analysis
        #
        MF.sTimer('PodStats    ---- ')
    ...
    if(not(ptype == 'pod' or ptype == 'pof')):
        ...
        rc=getStats(ptype,taus,taids,tstmids,vds,
                    filterdtgopt=filterdtgopt,
                    B2DSs=B2DSs,
                    doland=doland,
...
if(doHistplot != -999):
    ...
    lista=ovars2[ovars2.keys()[0]]['pe']
    listb=ovars2[ovars2.keys()[0]]['fe']
    ...
    ptitle2="scat-fe-%s-%03d-%s"%(aidopt,doHistplot,stmopt)
    pngpath="%s/%s.png"%(w2.ptmpBaseDir,ptitle2)
    
    rc=pltScatter(list1,vmax0,stmopt,vmax0,
                  xmin,xmax,ymin,ymax,
                  pngpath)
...
def setByStormPlot(ptype,pltcntvar=None):
    ...
    pltcntvar=None
    if(ptype == 'pe-imps' or ptype == 'fe-imps'):
        if(tauopt == '120'):
            pltcntvar='-20,20,5,2'
        else:
            pltcntvar='-20,10,5,2'
...
if(doplot):
    ...
    #if(ptype == 'gainxyfe' or ptype == 'gainxype' or ptype == 'pod'): doErrBar=0  # haven't figured out how to do box/whiskers for gainxy plots

    # -- plot controls in vdVM.py
    #
    rc=setVD2PlotVars(ptype,pcase,toptitle1=toptitle1,toptitle2=toptitle2)
    (pverikey,pverikey1,do1stplot,do2ndplot,do2ndval,pdoErrBar,optitle1,toptitle2)=rc
    
    # -- 20201219 -- force set of error bar at cmd line, e.g., -E0 to turn off
    #
    if(doErrBar == None):
        doErrBar=pdoErrBar
    ...
    for n in range(0,len(taids)):
        ...
        if(doBystorm):
            ...
            pltcntvar=setByStormPlot(ptype,pltcntvar)
    ...
    pss=SumStatsPlot(ss.models,ss.vstmids,pcase,ptype,pdir=pdir,doland=doland,doMetric=doMetric)
    if(verb): pss.ls()
    ...
    #plotcontrolVar=([0.0,150.0,25],2) #-- pod/poo
    #plotcontrolVar=([0.0,100.0,10],2) #-- 00-24 pe
